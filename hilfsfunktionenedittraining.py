'''
# hilfsfunktionenedittraining.py:
...
# --- Importiere die normalize_path_slashes Funktion ---
# Annahme: utils.py liegt im übergeordneten Verzeichnis (Projekt-Root)
import sys
import inspect
...
from utils import normalize_path_slashes # <--- Diese Zeile wurde HINZUGEFÜGT
...
def save_uploaded_file(uploaded_file, file_prefix, workout_name):
    ...
    if uploaded_file is not None:
        ...
        file_path = normalize_path_slashes(raw_file_path) # <--- HIER ANGEWENDET
        
        try:
            with open(raw_file_path, "wb") as f: # Beim Schreiben muss der OS-spezifische Pfad verwendet werden
                f.write(uploaded_file.getbuffer())
            return file_path # Rückgabe des normalisierten Pfades
        except Exception as e:
            st.error(f"Fehler beim Speichern der Datei {uploaded_file.name}: {e}")
            return None
    return None
...
def parse_gpx_data(gpx_file_path):
    ...
    # GPX-Dateipfad normalisieren, falls er aus einer nicht-normalisierten Quelle kommt (unwahrscheinlich hier, aber sicher ist sicher)
    # Beachten Sie, dass gpxpy selbst mit OS-spezifischen Pfaden umgehen kann.
    # Hier geht es eher darum, dass der Pfad, der hier reinkommt, wenn er aus der DB kommt,
    # bereits normalisiert sein sollte oder hier normalisiert wird, falls er anders erstellt wurde.
    # Für das Öffnen der Datei braucht Python den OS-spezifischen Pfad.
    ...
    try:
        with open(gpx_file_path_os_native, 'r') as gpx_file: # Öffnen mit dem OS-nativen Pfad
            gpx = gpxpy.parse(gpx_file)
...
def parse_fit_data(fit_file_path):
    ...
    # FIT-Dateipfad normalisieren
    fit_file_path_os_native = os.path.normpath(fit_file_path) # <--- HIER AUCH ANGEWENDET

    if not os.path.exists(fit_file_path_os_native):
        return 0, 0.0, None, None, 0, 0.0, 0, 0

    try:
        fitfile = FitFile(fit_file_path_os_native) # Öffnen mit dem OS-nativen Pfad

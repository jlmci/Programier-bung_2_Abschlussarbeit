...
def parse_gpx_data(gpx_file_path):
    """
    Parst eine GPX-Datei und extrahiert Dauer und Distanz.

    Args:
        gpx_file_path (str): Der Pfad zur GPX-Datei.

    Returns:
        tuple: (duration_minutes, total_distance_km) oder (0, 0.0) bei Fehler.
    """
    duration_minutes = 0
...
def display_workout_form(initial_data=None, form_key_suffix="add"):
    ...
    if st.session_state.get(f"{prefix}last_loaded_id_check") != initial_doc_id:
        ...
        st.session_state[f"{prefix}dauer_input"] = int(initial_data.get('dauer', 0)) if is_edit_mode else 0
    ...
    if uploaded_gpx_file is None:
        st.session_state[f"{prefix}dauer_input"] = 0
        st.session_state[f"{prefix}distanz_input"] = 0.0

    # Button zum Parsen der GPX-Daten, ebenfalls außerhalb des Formulars
    if st.button("GPX-Datei auswerten", key=f"{prefix}parse_gpx_button"):
        if uploaded_gpx_file:
            ...
            if duration_minutes > 0:
                st.session_state[f"{prefix}dauer_input"] = duration_minutes
    ...
    with st.form(key=f"{prefix}workout_form"):
        ...
        dauer = st.number_input("Dauer (in Minuten)", min_value=0, step=1, value=st.session_state.get(f"{prefix}dauer_input", 0), key=f"{prefix}dauer_input_form")
        ...
        #st.session_state[f"{prefix}selected_antrengung"] = None
        #st.session_state[f"{prefix}selected_star_rating"] = None

...
with st.form("add_profile_form"):
    ...
    #new_ekg_tests_str = st.text_input("Bereits vorEKG Test IDs (optional, kommagetrennt, z.B. 1,5,10):", value="", key="add_ekg_tests")
...
# --- Funktion zum Schreiben in die config.yaml ---
# Diese Funktion wird erweitert, um auch die Whitelist zu aktualisieren
def write_to_config_yaml(new_username, new_user_details, add_to_whitelist=False, new_doc_id=None):
    try:
        ...
        # NEU: Logik zum Hinzufügen zur Whitelist
        if add_to_whitelist and new_doc_id is not None:
            ...
            # Stelle sicher, dass die ID nicht bereits vorhanden ist
            if new_doc_id not in config_data['permissions']['can_add_profile_doc_ids']:
                ...
                # Sortieren der Liste, um sie sauber zu halten
...
with st.form("add_profile_form"):
    ...
    # NEU: Checkbox für Admin Whitelist
    ...
    if submit_button:
        ...
        if not new_firstname or not new_lastname or not new_username or not new_password or not confirm_password:
            ...
            try:
                ...
                # 4. Daten in config.yaml schreiben
                # NEU: Übergebe den Wert der Checkbox und die neue doc_id an die Schreibfunktion
                ...
                st.error(f"Ein unerwarteter Fehler ist aufgetreten: {e}")
        #st.switch_page("pages/Profil.py")  # Seite neu laden, um Formular zurückzusetzen
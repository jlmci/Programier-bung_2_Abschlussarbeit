...
with personendaten_col:
    ...
    if st.button("Allgemeine Änderungen speichern"):
        try:
            ...
            # --- NEUE FUNKTIONALITÄT: Update config.yaml with new name ---
            ...
            st.success("Allgemeine Personen-Informationen erfolgreich gespeichert!")
            st.rerun() # Rerun to reflect changes
        except Exception as e:
            st.error(f"Ein unerwarteter Fehler beim Speichern der allgemeinen Informationen ist aufgetreten: {e}")

    st.markdown("---")
    st.subheader("Login-Informationen (config.yaml)")
    ...
    if current_username_in_config:
        ...
        st.write(f"**Aktueller Benutzername:** `{current_username_in_config}`")

        # Display password for admins
        if st.session_state["admin"]:
            st.info(f"**Aktuelles Passwort (für Admin sichtbar):** `{user_config_entry.get('password', 'Nicht gesetzt')}`")
        ...
        with st.form("change_login_form"):
            ...
            confirm_current_password = st.text_input("Aktuelles Passwort bestätigen:", type="password")
    ...
    if current_username_in_config:
        ...
        with st.form("change_login_form"):
            ...
            if st.session_state["admin"]:
                ...
                st.info("Das aktuelle Passwort wurde automatisch eingefügt.") # Added a hint

...
def get_absolute_path(relative_path):
    ...
    potential_upload_path = os.path.join(os.path.dirname(__file__), UPLOAD_DIR, os.path.basename(relative_path))
    if os.path.exists(potential_upload_path):
        return potential_upload_path
    
    potential_image_path = os.path.join(os.path.dirname(__file__), IMAGE_DIR, os.path.basename(relative_path))
    if os.path.exists(potential_image_path):
        return potential_image_path

    potential_data_path = os.path.join(os.path.dirname(__file__), DATA_DIR, os.path.basename(relative_path))
...
def display_training_details_ui(training_data, on_delete_callback, on_edit_callback, expanded=False):
    ...
    with st.expander(expander_title, expanded=expanded):
        ...
        if image_path and image_path != "-": # Stellt sicher, dass ein Pfad vorhanden ist
            local_image_path = get_absolute_path(image_path) # <<< Hier wird die Magie der Pfadauflösung vollbracht
            if local_image_path and os.path.exists(local_image_path):
                st.image(local_image_path, caption=f"Bild für {training_data['name']}", use_container_width=True)
            else:
...
def edit_training_ui(training_data):
    ...
    with st.form(key=f"edit_training_form_{training_data.doc_id}"):
        ...
        anstrengung_labels = list(anstrengung_options_map.values())
        anstrengung_keys = list(anstrengung_options_map.keys())
        
        # Finde den Index des aktuell ausgewählten Werts
        initial_anstrengung_index = None
        if st.session_state.edit_selected_antrengung in anstrengung_keys:
            initial_anstrengung_index = anstrengung_keys.index(st.session_state.edit_selected_antrengung)

        # Verwende st.radio für die Anstrengung
        # Option index_to_value_map for st.radio is useful here
        # We need to map the stored key ('good', 'ok' etc.) to the display label
        selected_anstrengung_label = st.radio(
            "Wähle die Anstrengung:",
            options=anstrengung_labels,
            index=initial_anstrengung_index, # Setze den initialen Index
            key=f"edit_anstrengung_radio_{training_data.doc_id}",
            horizontal=True # Zeigt die Optionen nebeneinander an
        )
        # Speichere den Key zurück in den Session State
        st.session_state.edit_selected_antrengung = anstrengung_keys[anstrengung_labels.index(selected_anstrengung_label)]
        ...
        # Finde den Index des aktuell ausgewählten Stern-Werts
        initial_star_index = None
        if st.session_state.edit_selected_star_rating is not None and 1 <= st.session_state.edit_selected_star_rating <= 5:
            initial_star_index = st.session_state.edit_selected_star_rating - 1

        # Verwende st.radio für die Sternebewertung
        selected_star_label = st.radio(
            "Wähle die Bewertung:",
            options=star_options,
            index=initial_star_index, # Setze den initialen Index
            key=f"edit_star_radio_{training_data.doc_id}",
            horizontal=True # Zeigt die Optionen nebeneinander an
        )
        # Speichere den numerischen Wert zurück in den Session State
        st.session_state.edit_selected_star_rating = star_options.index(selected_star_label) + 1
        ...
        if submit_button:
            ...
            updated_training = {
                ...
                "puls": edited_puls,
                "kalorien": edited_kalorien,
                "anstrengung": st.session_state.edit_selected_antrengung,
                "star_rating": st.session_state.edit_selected_star_rating,
                "description": edited_description,
                "image": new_image_path,
                "gpx_file": new_gpx_path,
                "ekg_file": new_ekg_path,

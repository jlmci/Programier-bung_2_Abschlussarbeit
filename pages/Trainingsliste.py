...
def find_best_effort(df, window_size, power_col="power"):
    ...
    return int(max_value.max()) if not max_value.empty else None
...
def create_power_curve(df, power_col="power"):
    ...
    # Die Liste muss so angepasst werden, dass sie nicht über die Länge des Dataframes hinausgeht
...
def plot_power_curve(power_curve_df):
    ...
    fig.update_layout(
        ...
        template="plotly_white" # Ändere zu white für bessere Lesbarkeit in Streamlit
    )
    return fig
...
def display_fit_data_ui(fit_df, training_id_for_key):
    ...
    # Checkboxen für die Auswahl der Diagramme mit eindeutigen Keys
    st.markdown("Wähle die anzuzeigenden FIT-Diagramme:")
    col1, col2, col3, col4 = st.columns(4)
    with col1:
        show_hr = st.checkbox("Herzfrequenz", value=False, key=f"show_hr_checkbox_{training_id_for_key}")
    with col2:
        show_power = st.checkbox("Leistung", value=False, key=f"show_power_checkbox_{training_id_for_key}")
    with col3:
        show_velocity = st.checkbox("Geschwindigkeit", value=False, key=f"show_velocity_checkbox_{training_id_for_key}")
    with col4:
        show_cadence = st.checkbox("Trittfrequenz", value=False, key=f"show_cadence_checkbox_{training_id_for_key}")

    if show_hr and 'heart_rate' in fit_df.columns and fit_df['heart_rate'].dropna().any():
        ...
        st.info("Keine Herzfrequenzdaten in der FIT-Datei gefunden oder ausgewählt.")

    if show_power and 'power' in fit_df.columns and fit_df['power'].dropna().any():
        ...
        st.info("Keine Leistungsdaten in der FIT-Datei gefunden oder ausgewählt.")

    if show_velocity and 'velocity' in fit_df.columns and fit_df['velocity'].dropna().any():
        ...
        st.info("Keine Geschwindigkeitsdaten in der FIT-Datei gefunden oder ausgewählt.")

    if show_cadence and 'cadence' in fit_df.columns and fit_df['cadence'].dropna().any():
        ...
        st.info("Keine Trittfrequenzdaten in der FIT-Datei gefunden oder ausgewählt.")
...
def display_training_details_ui(training_data, on_delete_callback, on_edit_callback, expanded=False):
    ...
    with st.expander(expander_title, expanded=expanded):
        ...
        # Anpassung der Daueranzeige in Stunden und Minuten
        ...
        if fit_data_df is not None and not fit_data_df.empty:
            ...
            # Übergeben der Trainings-ID an die Funktion, um eindeutige Keys zu generieren
            display_fit_data_ui(fit_data_df, training_data.doc_id) 
        else:
...
def display_fit_map_ui(fit_df, training_id_for_key):
    ...
    # Entferne den 'key'-Parameter komplett
    folium_static(m) 
...
def display_fit_data_ui(fit_df, training_id_for_key):
    ...
    if has_gps_data:
        checkboxes.append(("Strecke (FIT-Karte)", True, f"show_fit_map_{training_id_for_key}"))

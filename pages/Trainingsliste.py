...
def load_ekg_data(ekg_filepath):
    """
    Lädt EKG-Daten aus einer TXT- oder CSV-Datei und erstellt ein EKGdata-Objekt.
    Gibt das EKGdata-Objekt zurück oder None bei Fehlern.
    """
    abs_filepath = ekg_filepath 
    
    if not abs_filepath or not os.path.exists(abs_filepath):
        # Removed st.write here to avoid redundant messages, display_ekg_data_ui handles this
        return None
...
def display_ekg_data_ui(ekg_obj, training_id_for_key):
    """
    Zeigt das EKG-Diagramm mit einem Zeitbereichs-Slider an.
    """
    ...
    # Berechne den maximalen Zeitbereich in Sekunden
    ...
    # Slider für den Zeitbereich
    # Setze einen sinnvollen Standardbereich, z.B. die ersten 10 Sekunden oder den gesamten Bereich, wenn kurz
    ...
    # Filter das DataFrame basierend auf dem ausgewählten Zeitbereich
    ...
    # Füge die Haupt-EKG-Messwerte als Scatter-Trace hinzu
    fig.add_trace(
        go.Scatter(
            x=filtered_df["Zeit in s"],
            y=filtered_df["Messwerte in mV"],
            mode="lines",
            name="EKG-Messwerte", # <-- Hier definieren wir den Namen für die Legende
            line=dict(color="skyblue") # Optional: Farbe explizit setzen (entspricht deiner blauen Linie)
        )
    )
    ...
    # Peaks für den gefilterten Bereich finden (müssen auf dem originalen Index basieren)
    # Da find_peaks auf self.df arbeitet, müssen wir die Peaks nach dem Filtern des Plots anpassen.
    # Oder wir finden Peaks auf dem gesamten Datensatz und filtern dann die angezeigten Peaks.
    # Letzteres ist robuster, da der Schwellenwert auf dem gesamten Datensatz berechnet wird.
    all_peaks = ekg_obj.find_peaks(respacing_factor=5)
    
    # Filtere Peaks, die im aktuellen Zeitbereich liegen
    peak_times_all = ekg_obj.df.loc[all_peaks, "Zeit in s"]
    peak_values_all = ekg_obj.df.loc[all_peaks, "Messwerte in mV"]

    # Finde Indizes der Peaks, die im aktuellen Slider-Bereich liegen
    peaks_in_view_indices = peak_times_all[
        (peak_times_all >= start_time) & (peak_times_all <= end_time)
    ].index.tolist()

    if peaks_in_view_indices:
        # Extrahiere Zeit- & Messwertwerte der Peaks im sichtbaren Bereich
